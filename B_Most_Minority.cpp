...
#define  msll	     map<ll,unordered_set<ll>> //Storing Graphs
...
int main()
{
    ...
    while(t--){
        ...
        for (int col = 0; col < m; col++)
        {
            ...
            if(cnt_o==0 || cnt_z==0) {
                for (int row = 0; row < n; row++) scores[row]++;
            }
            else if (cnt_o < cnt_z) { 
                for (int row = 0; row < n; row++) {
                    if (v[row][col] == '1') scores[row]++;
                }
            } 
            else { 
                for (int row = 0; row < n; row++) {
                    if (v[row][col] == '0') scores[row]++;
                }
            }
        }
        msll mpx;
        for (int i = 0; i < n; i++) {
            mpx[scores[i]].insert(i+1);
        }
        auto &bestSet = mpx.rbegin()->second;
