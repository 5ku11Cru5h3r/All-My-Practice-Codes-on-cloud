...
bool comp(tuple<ll,ll,ll> f,tuple<ll,ll,ll> s){
    if(f==s) return false;
    if(f<s){swap(f,s);}
    ll a=get<0>(f),b=get<1>(f),c=get<2>(f);
    ll aa=get<0>(s),bb=get<1>(s),cc=get<2>(s);
...
int main()
{
    ...
    while(t--){
        ...
        // for(auto &[i,j,k]:v) cout<<i<<' '<<j<<' '<<k<<endl;
        bool flag=(comp(v[0],v[1]) & comp(v[1],v[2]) & comp(v[0],v[2]));
