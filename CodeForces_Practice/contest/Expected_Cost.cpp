...
int main()
{
    ...
    while(t--){
        ...
        auto f=[&](long double x)->pair<long double,long double>{
            long double sum=0,curr=0;
            for(int i=n-1; i>=0;i--){
                curr=min(curr+v[i].first,x/(n+1)+v[i].second);
                sum+=curr;
            }
            return {sum,curr};
        };
        
        long double high=1e18,lo=0;
        for (size_t i = 0; i < 100; i++)
        {
            long double mid=(lo+high)/2.0L;
            auto [x,y]=f(mid);
            if(x<mid) high=mid;
            else lo=mid;
        }
        ...
        cout<<fixed<<setprecision(6)<<f(lo).second<<endl;
    }
    return 0;
}